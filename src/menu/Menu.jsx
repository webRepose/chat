...
const Menu = () => {
    ...
    const addNewChat = async(user1, user2, user2Photo, user2DisplayName) => {      
        let resChat = true;
        checkHaveChat && checkHaveChat.forEach(e => {
    ...
    return (
        <main style={{height: '98vh'}}>
            <Section>
                {modalChoiceUser &&
                    <div className={Style.modal}>
                        <div className={Style.modal_choice}>
                            ...
                            {usersAll && usersAll.map((data, id) => (
                                user.uid !== data.uid &&
                                    <div onClick={() => {
                                            addNewChat(user.uid, data.uid, data.photoURL, data.displayName);
                                            setModalChoiceUser(prev => prev = false);
                                        }} className={Style.modal_choice_data} key={id}>
                                            <img width={50} src={data.photoURL && data.photoURL} alt="avatar"/>
                ...
                <ChatList/>            
            <button onClick={()=>{setModalChoiceUser(prev => prev = true)}} className={Style.menu_addNew}>Добавить чат</button>
            </Section>
        </main>
    );
};
